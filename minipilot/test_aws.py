...
def _vit_self_attn_est(model, B: int = 1) -> int:
    """ViT 自己注意スコアの推定 FLOPs (QK^T + A@V, フック計測外)。非 ViT backbone は 0 を返す。"""
    ...
    try:
        ...
        return 4 * B * num_heads * N * N * head_dim * len(blocks)
...
def print_model_stats(sv, sv_teacher=None) -> None:
    ...
    # ── TinySteerViT パラメータ表 ────────────────────────────────────────
    vis_name = getattr(sv, "vision_encoder_name", "vit_tiny")
    d_vis    = sv.vision_model.trunk.embed_dim
    n_gca    = len(sv.vision_model.gated_cross_attn)
    _print_param_table(f"TinySteerViT ({enc_name} + {vis_name})", [
        (f"text_model ({enc_name}, frozen)",                        sv.text_model),
        (f"connector  (Linear {d_text}→{d_vis})",                   sv.connector),
        (f"vision_model.trunk ({vis_name}, frozen)",                sv.vision_model.trunk),
        (f"gated_cross_attn  ({n_gca} × GatedCrossAttn)",          sv.vision_model.gated_cross_attn),
        (f"lin_seg_head (Linear {d_vis}→1)",                        sv.lin_seg_head),
    ])
    ...
    dummy_imgs   = torch.zeros(B_d, 3, sv.img_size, sv.img_size,
                               device=device_tiny)
    ...
    print(f"\n  TinySteerViT FLOPs  (B={B_d}, img={sv.img_size}px, "
          f"text_seq={L_d})")
...
def parse_args():
    ...
    p.add_argument("--log", type=str, default=None,
                   help="結果を保存する JSON ファイルパス (省略時は自動生成)")
...
def _save_log(
    ...
    log = {
        ...
        "val_loss":  ckpt_info.get("val_loss"),
        "train_args": ckpt_info.get("args", {}),
...
def main():
    ...
    if log_path is None:
        ...
        log_path = f"test_log_{stem}_{ts}.json"
    txt_log_path = str(Path(log_path).with_suffix(".log"))
    tee = _Tee(txt_log_path)
    ...
    for split in splits:
        ...
        loader_tiny = _make_loader(
            args.dataset, split, tiny.img_size,
    ...
    _save_log(log_path, args, ckpt_info, all_results, splits)
